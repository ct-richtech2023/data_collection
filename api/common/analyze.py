...
class McapReader(QObject):
    ...
    def save_frame_as_image(self, frame, output_path="frame.jpg"):
        """将帧数据保存为图像文件
        
        Args:
            frame: numpy数组格式的图像数据
            output_path: 输出文件路径
        """
        if frame is not None:
            ...
            logger.warning("帧数据为空，无法保存")
            return False

    def display_frame_info(self, frame, frame_index=0):
        """显示帧的详细信息
        ...
        """
        if frame is not None:
            logger.info(f"帧 {frame_index} 信息:")
            logger.info(f"  数据类型: {type(frame)}")
            logger.info(f"  数组形状: {frame.shape}")
            logger.info(f"  数据类型: {frame.dtype}")
            logger.info(f"  像素值范围: [{frame.min()}, {frame.max()}]")
            logger.info(f"  内存大小: {frame.nbytes} bytes")
            
            if len(frame.shape) == 3:
                logger.info(f"  图像类型: 彩色图像 ({frame.shape[2]} 通道)")
                logger.info(f"  分辨率: {frame.shape[1]}x{frame.shape[0]}")
            elif len(frame.shape) == 2:
                logger.info(f"  图像类型: 灰度图像")
                logger.info(f"  分辨率: {frame.shape[1]}x{frame.shape[0]}")
        else:
            logger.warning(f"帧 {frame_index} 数据为空")
...
def test_mcap_file(mcap_path: str, test_name: str = "MCAP文件"):
    """测试单个MCAP文件的基本功能"""
    logger.info(f"开始测试 {test_name}: {mcap_path}")
    ...
    try:
        ...
        # 6. 测试图像查看功能
        logger.info("测试图像查看功能...")
        reader.start_load_video()
        ...
        while True:
            # 获取第一帧进行测试
            test_frame = reader.get_next_frame()
            if test_frame and 'topics' in test_frame:
                logger.info("获取到测试帧，分析图像数据:")
                
                for topic, img_data in test_frame['topics'].items():
                    if img_data is not None:
                        logger.info(f"  Topic: {topic}")
                        reader.display_frame_info(img_data, 0)
                        
                        # 保存图像文件
                        output_file = f"code/analyze_mcap/images/test_frame_{i}_{topic.replace('/', '_')}.jpg"
                        reader.save_frame_as_image(img_data, output_file)
                        
                        # # 显示如何查看图像
                        # logger.info(f"  查看图像方法:")
                        # logger.info(f"    1. 文件已保存: {output_file}")
                        # logger.info(f"    2. 使用OpenCV: cv2.imshow('image', img_data)")
                        # logger.info(f"    3. 使用Matplotlib: plt.imshow(img_data)")
                        # break
            else:
                logger.warning("获取到测试帧失败")
                break
            i += 1
...
if __name__ == "__main__":
    ...
    # 测试用例 - 使用绝对路径
    file1 = script_dir / "20250817_144725.mcap"

    # 检查文件是否存在
    if not file1.exists():
        logger.error(f"文件1不存在: {file1}")
        logger.info(f"当前目录: {script_dir}")
        logger.info("请确保MCAP文件在当前目录中")
    
    test_mcap_file(file1)